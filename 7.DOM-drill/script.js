...
for (let i = 1; i < childNodesFromList.length; i += 2) {
  ...
  if (
    childNodesFromList[i].innerHTML === "Fast and Furious" &&
    nrOfItems != 0
  ) {
  ...
  for (let item of childNodesFromList) {
    if (childNodesFromList[i].isEqualNode(item)) {
      // childNodesFromList[i].parentNode.removeChild(childNodesFromList[i]);
      // console.log("Node: " + childNodesFromList[i].textContent);
...
const allFranchises = document.createElement("option");
const importantFranchises = document.createElement("option");
const normalFranchises = document.createElement("option");

allFranchises.appendChild(document.createTextNode("All Franchises"));
importantFranchises.appendChild(
  document.createTextNode("Important Franchises")
);
normalFranchises.appendChild(document.createTextNode("Normal Franchises"));
selectFranchises.appendChild(allFranchises);
...
selectFranchises.addEventListener("change", (event) => {
  if (event.target.value === "Important Franchises") {
    for (let i of uList.children) {
      i.classList.contains("important")
...
const firstItemFF = uList.children[0];
document.body.addEventListener("keyup", (event) => {
  if (event.code === "KeyR") {
    const random = Math.floor(Math.random() * uList.children.length);

    for (let listItem of uList.children) {
      if (
        listItem.classList.contains("important") ||
        listItem.innerHTML === "Fast and Furious"
      ) {
        continue;
      } else {
        uList.insertBefore(listItem, uList.children[random]);
      }
    }
    uList.insertBefore(firstItemFF, uList.children[0]);
  }
});
...
document.body.addEventListener("keyup", (event) => {
  if (event.code === "KeyD") {
    const newFirstItemFF = uList.children[0].cloneNode(true);
    uList.prepend(newFirstItemFF);
    // uList.insertBefore(newFirstItemFF, );
  }
});
